...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void initStrategies(ApplicationContext context) {
		initMultipartResolver(context); // 文件上传解析器（将普通的request包装成MultipartHttpServletRequest）
		initLocaleResolver(context); // 国际化
		initThemeResolver(context); // 主题
		initHandlerMappings(context); // HandlerMapping
		initHandlerAdapters(context); // 适配器（适配servlet和controller method）
		initHandlerExceptionResolvers(context); // 异常处理
		initRequestToViewNameTranslator(context); // 从request中获取viewName
		initViewResolvers(context); // 渲染页面
		initFlashMapManager(context); // 管理FlashMap（重定向中传递参数）
	}
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// 最终 反射调用controller实现 也就是让适配器去执行MappingHandler
	...
	protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		if (this.handlerMappings != null) {
			for (HandlerMapping mapping : this.handlerMappings) {
				// HandlerExecutionChain只能通过HandlerMapping中的getHandler获取
