...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			// 扫描basePackage路径下的java文件 全部转为BeanDefinition 然后再判断拿出符合条件的beanDefinition
			// ASM实现
			// 扫描出来的BD是ScannedGenericBeanDefinition
			Set<BeanDefinition> candidates = findCandidateComponents(basePackage);
			// 遍历所有beanDefinition并将其放入beanFactory的beanDefinitionMap中
			for (BeanDefinition candidate : candidates) {
				// 解析scope
				ScopeMetadata scopeMetadata = this.scopeMetadataResolver.resolveScopeMetadata(candidate);
				candidate.setScope(scopeMetadata.getScopeName());
				// 获取beanName 先判断注解上是否设置beanName 如果没就以小写为beanName
				String beanName = this.beanNameGenerator.generateBeanName(candidate, this.registry);
				if (candidate instanceof AbstractBeanDefinition) {
					// ScannedGenericBeanDefinition是AbstractBeanDefinition类型 设置默认值 比如lazy init destroy
					postProcessBeanDefinition((AbstractBeanDefinition) candidate, beanName);
				}
				if (candidate instanceof AnnotatedBeanDefinition) {
					// 处理加了注解的类 把常用注解设置到AnnotationBeanDefinition中
				...
				if (checkCandidate(beanName, candidate)) {
					...
					//  最终将beanDefinition注册到了DefaultListableBeanFactory中
