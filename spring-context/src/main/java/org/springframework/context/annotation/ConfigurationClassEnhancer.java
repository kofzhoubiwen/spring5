...
class ConfigurationClassEnhancer {
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		// 通过是否添加EnhancedConfiguration来判断是会否已经被代理过
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		// 增强接口
		// EnhancedConfiguration继承了BeanFactoryAware能拿到BeanFactory
		// 然后就可以通过BeanFactory.getBean("x")拿到代理对象
		enhancer.setInterfaces(new Class<?>[] {EnhancedConfiguration.class});
		enhancer.setUseFactory(false);
		// 命名策略 BySpringCGLIB
		enhancer.setNamingPolicy(SpringNamingPolicy.INSTANCE);
		// 暴露ClassLoader
		// 定义beanFactory属性名（$$beanFactory）这样cglib代理对象就有了beanFactory 这样就能从工厂中获得已缓存的bean对象了
	...
	private static class BeanFactoryAwareGeneratorStrategy extends DefaultGeneratorStrategy {
		...
		protected ClassGenerator transform(ClassGenerator cg) throws Exception {
			// 申请一个BeanFactory的属性$$beanFactory
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			// a(){b()},a方法中调用的b方法 那么此时调用b方法创建b对象时正在执行的就是a方法
			// 通过正在执行的方法和代理方法是否同名来确定
		...
		private Object resolveBeanReference(Method beanMethod, Object[] beanMethodArgs,
			...
			try {
				...
				// 在调用beanMethod的时候，也就是被@Bean注解标注的方法的时候如果使用了参数，只要有一个参数为null,
				// 就直接调用getBean(beanName),否则带参数调用getBean(beanName,args)，后面通过例子解释这段代码
				...
				// 这里发现getBean返回的类型不是我们方法返回的类型 说明beanMethod所定义的bd被覆盖了
