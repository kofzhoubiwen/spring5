...
public class QualifierAnnotationAutowireCandidateResolver extends GenericTypeAwareAutowireCandidateResolver {
	...
	protected boolean checkQualifier(
		...
		if (qualifier == null) {
			...
			// 1. 此方法是从bd标签里拿这个类型的注解声明 非XML配置时代此处targetAnnotation 为null
			Annotation targetAnnotation = getQualifiedElementAnnotation(bd, type);
			// Then, check annotation on factory method, if applicable
			// 2. 若为null 去工厂方法里拿这个类型的注解 这方法里标注了两个注解@Bean和@LoadBalanced
			// 所以此时targetAnnotation就不再为null了
			...
			// 若xml 工厂方法 父里都还没找到此方法 回退到类本身上去看
			// 也就是说 如果@LoadBalanced标注在RestTemplate上 也是可以的
			if (targetAnnotation == null) {
		...
		// 若targetAnnotation还没找到 也就是还没匹配上
		// !! 仍旧还不放弃 拿到当前这个注解的所有注解属性继续尝试匹配
