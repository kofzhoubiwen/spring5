...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		// context.getBean("xx",Object... args)
		...
		if (constructorToUse == null || argsToUse == null) {
			...
			// 如果传入的构造器不为空 或者 容器指定的注入模式是按构造方法注入（AUTOWIRE_CONSTRUCTOR）
			...
			if (explicitArgs != null) {
				...
				// 这边解析mbd中的构造函数参数值 主要是处理我们通过xml方式定义的构造函数注入的参数 获 getBean时候指定参数
				// ConstructorArgumentValues内部定义了一个map和一个list来保存构造方法的参数
				// <constructor-arg index="0" value="Vincent" /> 当指定index的时候使用map来存放
				// <constructor-arg name="age" value="30" /> 当参数无序的时候使用list来存放
			...
			for (Constructor<?> candidate : candidates) {
				...
				// constructorToUse != null 构造方法已明确 且
				// argsToUse != null 构造参数已明确 且
				// argsToUse.length > paramTypes.length 确定的参数个数比当前参数个数多的时候
				// 跳过

				if (constructorToUse != null && argsToUse != null && argsToUse.length > paramTypes.length) {
					// Already found greedy constructor that can be satisfied ->
					// do not look any further, there are only less greedy constructors left.
					// 已经找到合适的构造方法了 没必要在继续 break就OK了
					break;
				}
				if (paramTypes.length < minNrOfArgs) {
					// 如果当前构造方法的参数个数小于最小参数值 则进行下一轮循环
					continue;
				}
				...
				if (typeDiffWeight < minTypeDiffWeight) {
					...
					// 如果出现权重值更小的候选者 则将ambiguousConstructors清空
					// 这也是为什么之前找到相同差异值的时候不是直接抛出异常 而是先保存下来的原因
			...
			if (constructorToUse == null) {
				...
				// 如果找到了匹配的构造函数
				// 但是存在多个（ambiguousConstructors不为空） && 解析构造函数的模式为严格模式 则抛出异常
				throw new BeanCreationException(mbd.getResourceDescription(), beanName,
